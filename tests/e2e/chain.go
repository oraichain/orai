...
import (
	...
	orai "github.com/oraichain/orai/app"
...
func init() {
	encodingConfig = orai.MakeEncodingConfig()
...
func newChain() (*chain, error) {
	tmpDir, err := os.MkdirTemp("", "orai-e2e-testnet-")
	if err != nil {
		return nil, err
	}
...
func (c *chain) createValidator(index int) *validator {
	return &validator{
		chain:   c,
		index:   index,
		moniker: fmt.Sprintf("%s-orai-%d", c.id, index),
	}
}
