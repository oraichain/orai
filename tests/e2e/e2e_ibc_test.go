...
func (s *IntegrationTestSuite) runIBCRelayer() {
	...
	tmpDir, err := os.MkdirTemp("", "orai-e2e-testnet-hermes-")
	s.Require().NoError(err)
	s.tmpDirs = append(s.tmpDirs, tmpDir)

	oraiAVal := s.chainA.validators[0]
	oraiBVal := s.chainB.validators[0]

	oraiARly := s.chainA.genesisAccounts[relayerAccountIndex]
	oraiBRly := s.chainB.genesisAccounts[relayerAccountIndex]
	...
	s.hermesResource, err = s.dkrPool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("orai_A_E2E_CHAIN_ID=%s", s.chainA.id),
				fmt.Sprintf("orai_B_E2E_CHAIN_ID=%s", s.chainB.id),
				fmt.Sprintf("orai_A_E2E_VAL_MNEMONIC=%s", oraiAVal.mnemonic),
				fmt.Sprintf("orai_B_E2E_VAL_MNEMONIC=%s", oraiBVal.mnemonic),
				fmt.Sprintf("orai_A_E2E_RLY_MNEMONIC=%s", oraiARly.mnemonic),
				fmt.Sprintf("orai_B_E2E_RLY_MNEMONIC=%s", oraiBRly.mnemonic),
				fmt.Sprintf("orai_A_E2E_VAL_HOST=%s", s.valResources[s.chainA.id][0].Container.Name[1:]),
				fmt.Sprintf("orai_B_E2E_VAL_HOST=%s", s.valResources[s.chainB.id][0].Container.Name[1:]),
			},
			Entrypoint: []string{
				"sh",
				"-c",
	...
	// create the client, connection and channel between the two orai chains
	s.createConnection()
	time.Sleep(10 * time.Second)
	s.createChannel()
}
...
func (s *IntegrationTestSuite) sendIBC(c *chain, valIdx int, sender, recipient, token, fees, note string) {
	...
	s.executeoraiTxCommand(ctx, c, ibcCmd, valIdx, s.defaultExecValidation(c, valIdx))
...
func (s *IntegrationTestSuite) testIBCTokenTransfer() {
	...
	s.Run("send_uorai_to_chainB", func() {
		...
		s.Require().Eventually(
			func() bool {
				balances, err = queryoraiAllBalances(chainBAPIEndpoint, recipient)
				s.Require().NoError(err)
		...
		tokenAmt := 3300000000
		s.sendIBC(s.chainA, 0, sender, recipient, strconv.Itoa(tokenAmt)+uoraiDenom, standardFees.String(), "")
...
func (s *IntegrationTestSuite) testMultihopIBCTokenTransfer() {
	...
	s.Run("send_successful_multihop_uorai_to_chainA_from_chainA", func() {
		// require the recipient account receives the IBC tokens (IBC packets ACKd)
		var (
			err error
		)
		...
		s.Require().Eventually(
			func() bool {
				...
				s.Require().NoError(err)
				fmt.Println("beforeSenderuoraiBalance", beforeSenderuoraiBalance)

				beforeRecipientuoraiBalance, err = getSpecificBalance(chainAAPIEndpoint, recipient, uoraiDenom)
				s.Require().NoError(err)
				fmt.Print("beforeRecipientuoraiBalance", beforeRecipientuoraiBalance)
		...
		s.sendIBC(s.chainA, 0, sender, middlehop, strconv.Itoa(tokenAmt)+uoraiDenom, standardFees.String(), string(memo))
...
func (s *IntegrationTestSuite) testFailedMultihopIBCTokenTransfer() {
	...
	s.Run("send_failed_multihop_uorai_to_chainA_from_chainA", func() {
		address := s.chainA.validators[0].keyInfo.GetAddress()
		sender := address.String()
		...
		s.Require().Eventually(
			func() bool {
				...
				return beforeSenderuoraiBalance.IsValid()
			},
			1*time.Minute,
			5*time.Second,
		)
		...
		s.Require().Eventually(
			func() bool {
				...
				returned := beforeSenderuoraiBalance.Sub(tokenAmount).Sub(standardFees).IsEqual(afterSenderuoraiBalance)
		...
		s.Require().Eventually(
			func() bool {
				...
				returned := beforeSenderuoraiBalance.Sub(standardFees).IsEqual(afterSenderuoraiBalance)
