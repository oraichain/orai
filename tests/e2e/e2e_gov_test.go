...
func (s *IntegrationTestSuite) GovCommunityPoolSpend() {
	...
	sendAmount := sdk.NewCoin(uoraiDenom, sdk.NewInt(10000000)) // 10uorai
	s.writeGovCommunitySpendProposal(s.chainA, sendAmount.String(), recipient)

	beforeRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uoraiDenom)
	...
	s.Require().Eventually(
		func() bool {
			afterRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uoraiDenom)
			s.Require().NoError(err)
...
/*
AddRemoveConsumerChain tests adding and subsequently removing a new consumer chain to orai.
Test Benchmarks:
...
func (s *IntegrationTestSuite) AddRemoveConsumerChain() {
	fmt.Printf("[LOG] AddRemoveConsumerChain\n")
	s.fundCommunityPool()
	chainAAPIEndpoint := fmt.Sprintf("http://%s", s.valResources[s.chainA.id][0].GetHostPort("1317/tcp"))
	...
	s.execQueryConsumerChains(s.chainA, 0, oraiHomePath, validateConsumerAddition, consumerChainID)
	...
	s.execQueryConsumerChains(s.chainA, 0, oraiHomePath, validateConsumerRemoval, consumerChainID)
}
