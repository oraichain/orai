...
import (
	...
	// globalfee "github.com/cosmos/orai/v9/x/globalfee/types"
)

func queryoraiTx(endpoint, txHash string) error {
...
func getSpecificBalance(endpoint, addr, denom string) (amt sdk.Coin, err error) {
	balances, err := queryoraiAllBalances(endpoint, addr)
	if err != nil {
		return amt, err
	}
...
func queryoraiAllBalances(endpoint, addr string) (sdk.Coins, error) {
