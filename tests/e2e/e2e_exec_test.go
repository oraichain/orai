...
func (s *IntegrationTestSuite) execEncode(
	...
	s.T().Logf("%s - Executing oraid encoding with %v", c.id, txPath)
	oraiCommand := []string{
		oraidBinary,
		txCommand,
		"encode",
	...
	for flag, value := range opts {
		oraiCommand = append(oraiCommand, fmt.Sprintf("--%s=%v", flag, value))
	}

	var encoded string
	s.executeoraiTxCommand(ctx, c, oraiCommand, 0, func(stdOut []byte, stdErr []byte) bool {
		if stdErr != nil {
			return false
		}
...
func (s *IntegrationTestSuite) execDecode(
	...
	s.T().Logf("%s - Executing oraid decoding with %v", c.id, txPath)
	oraiCommand := []string{
		oraidBinary,
		txCommand,
		"decode",
...
func (s *IntegrationTestSuite) execVestingTx(
	...
	s.T().Logf("%s - Executing oraid %s with %v", c.id, method, args)
	oraiCommand := []string{
		oraidBinary,
		txCommand,
	...
	oraiCommand = append(oraiCommand, args...)

	for flag, value := range opts {
		oraiCommand = append(oraiCommand, fmt.Sprintf("--%s=%v", flag, value))
	}

	s.executeoraiTxCommand(ctx, c, oraiCommand, 0, s.defaultExecValidation(c, 0))
...
func (s *IntegrationTestSuite) execCreatePeriodicVestingAccount(
	...
	s.T().Logf("Executing oraid create periodic vesting account %s", c.id)
...
func (s *IntegrationTestSuite) execUnjail(
	...
	s.T().Logf("Executing oraid slashing unjail %s with options: %v", c.id, opt)
	oraiCommand := []string{
		oraidBinary,
		txCommand,
...
func (s *IntegrationTestSuite) execFeeGrant(c *chain, valIdx int, granter, grantee, spendLimit string, opt ...flagOption) {
	...
	s.executeoraiTxCommand(ctx, c, oraiCommand, valIdx, s.defaultExecValidation(c, valIdx))
}
...
func (s *IntegrationTestSuite) execBankSend(
	...
	s.executeoraiTxCommand(ctx, c, oraiCommand, valIdx, s.expectErrExecValidation(c, valIdx, expectErr))
}
...
func (s *IntegrationTestSuite) execDistributionFundCommunityPool(c *chain, valIdx int, from, amt, fees string) {
	...
	s.T().Logf("Executing oraid tx distribution fund-community-pool on chain %s", c.id)
...
func (s *IntegrationTestSuite) runGovExec(c *chain, valIdx int, submitterAddr, govCommand string, proposalFlags []string, fees string) {
	...
	oraiCommand = concatFlags(oraiCommand, proposalFlags, generalFlags)

	s.T().Logf("Executing oraid tx gov %s on chain %s", govCommand, c.id)
	s.executeoraiTxCommand(ctx, c, oraiCommand, valIdx, s.defaultExecValidation(c, valIdx))
...
func (s *IntegrationTestSuite) executeGKeysAddCommand(c *chain, valIdx int, name string, home string) string {
	...
	s.executeoraiTxCommand(ctx, c, oraiCommand, valIdx, func(stdOut []byte, stdErr []byte) bool {
		// oraid keys add by default returns payload to stdErr
...
func (s *IntegrationTestSuite) executeKeysList(c *chain, valIdx int, home string) {
	...
	s.executeoraiTxCommand(ctx, c, oraiCommand, valIdx, func([]byte, []byte) bool {
		return true
	})
}
...
func (s *IntegrationTestSuite) executeDelegate(c *chain, valIdx int, amount, valOperAddress, delegatorAddr, home, delegateFees string) { //nolint:unparam
	...
	s.T().Logf("Executing oraid tx staking delegate %s", c.id)
...
func (s *IntegrationTestSuite) executeRedelegate(c *chain, valIdx int, amount, originalValOperAddress,
	...
	s.T().Logf("Executing oraid tx staking redelegate %s", c.id)
...
func (s *IntegrationTestSuite) getLatestBlockHeight(c *chain, valIdx int) int {
	...
	oraiCommand := []string{oraidBinary, "status"}
	s.executeoraiTxCommand(ctx, c, oraiCommand, valIdx, func(stdOut []byte, stdErr []byte) bool {
		var (
			err   error
			block syncInfo
...
func (s *IntegrationTestSuite) verifyBalanceChange(endpoint string, expectedAmount sdk.Coin, recipientAddress string) {
	s.Require().Eventually(
		func() bool {
			afterAtomBalance, err := getSpecificBalance(endpoint, recipientAddress, uoraiDenom)
			s.Require().NoError(err)
...
func (s *IntegrationTestSuite) execWithdrawReward(
	...
	oraiCommand := []string{
		...
		validatorAddress,
		fmt.Sprintf("--%s=%s", flags.FlagFrom, delegatorAddress),
		fmt.Sprintf("--%s=%s", flags.FlagGasPrices, "300uorai"),
		fmt.Sprintf("--%s=%s", flags.FlagGas, "auto"),
...
func (s *IntegrationTestSuite) executeoraiTxCommand(ctx context.Context, c *chain, oraiCommand []string, valIdx int, validation func([]byte, []byte) bool) {
	if validation == nil {
...
func (s *IntegrationTestSuite) expectErrExecValidation(chain *chain, valIdx int, expectErr bool) func([]byte, []byte) bool {
	return func(stdOut []byte, stdErr []byte) bool {
		...
		s.Require().Eventuallyf(
			func() bool {
				gotErr := queryoraiTx(endpoint, txResp.TxHash) != nil
...
func (s *IntegrationTestSuite) defaultExecValidation(chain *chain, valIdx int) func([]byte, []byte) bool {
	return func(stdOut []byte, stdErr []byte) bool {
		...
		if strings.Contains(txResp.String(), "code: 0") || txResp.Code == 0 {
			...
			s.Require().Eventually(
				func() bool {
					return queryoraiTx(endpoint, txResp.TxHash) == nil
				},
				time.Minute,
				5*time.Second,
				"stdOut: %s, stdErr: %s",
