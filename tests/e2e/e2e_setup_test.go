...
const (
	...
	queryCommand   = "query"
	keysCommand    = "keys"
	oraiHomePath   = "/home/nonroot/.orai"
	photonDenom    = "photon"
	uoraiDenom     = "uorai"
	initBalanceStr = "110000000000stake,100000000000000000photon,100000000000000000uorai"
	minGasPrice    = "0.00001"
...
var (
	oraiConfigPath    = filepath.Join(oraiHomePath, "config")
	stakingAmount     = sdk.NewInt(100000000000)
	stakingAmountCoin = sdk.NewCoin(uoraiDenom, stakingAmount)
	tokenAmount       = sdk.NewCoin(uoraiDenom, sdk.NewInt(3300000000)) // 3,300uorai
	standardFees      = sdk.NewCoin(uoraiDenom, sdk.NewInt(330000))     // 0.33uorai
	depositAmount     = sdk.NewCoin(uoraiDenom, sdk.NewInt(10000000))   // 10uorai
...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	//
	// 1. Initialize orai validator nodes.
	// 2. Create and initialize orai validator genesis files (both chains)
...
func (s *IntegrationTestSuite) TearDownSuite() {
	if str := os.Getenv("orai_E2E_SKIP_CLEANUP"); len(str) > 0 {
...
func (s *IntegrationTestSuite) initNodes(c *chain) {
	...
	*/
	...
	// s.Require().NoError(
	// 	modifyGenesis(val0ConfigDir, "", initBalanceStr, addrAll, initialGlobalFeeAmt+uoraiDenom, uoraiDenom),
	// )
...
func (s *IntegrationTestSuite) addGenesisVestingAndJailedAccounts(
	...
	stakingModuleBalances := banktypes.Balance{
		...
		Coins:   sdk.NewCoins(sdk.NewCoin(uoraiDenom, sdk.NewInt(slashingShares))),
	}
...
func (s *IntegrationTestSuite) initValidatorConfigs(c *chain) {
	for i, val := range c.validators {
		...
		appConfig.MinGasPrices = fmt.Sprintf("%s%s", minGasPrice, uoraiDenom)
	}
}

// runValidators runs the validators in the chain
func (s *IntegrationTestSuite) runValidators(c *chain, portOffset int) {
	s.T().Logf("starting orai %s validator containers...", c.id)
	...
	for i, val := range c.validators {
		runOpts := &dockertest.RunOptions{
			...
			Mounts: []string{
				fmt.Sprintf("%s/:%s", val.configDir(), oraiHomePath),
			},
			Repository: "cosmos/oraid-e2e",
		}
		...
		s.T().Logf("started orai %s validator container: %s", c.id, resource.Container.ID)
	}
	...
	s.Require().Eventually(
		...
		5*time.Minute,
		time.Second,
		"orai node failed to produce blocks",
	)
}
...
func configFile(filename string) string {
	filepath := filepath.Join(oraiConfigPath, filename)
	return filepath
}
