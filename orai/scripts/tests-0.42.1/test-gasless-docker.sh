...
ARGS="--from $USER --chain-id testing -y --keyring-backend test --fees 200orai --gas 20000000 --gas-adjustment 1.5 -b block --home $NODE_HOME"
...
$validator1_command "oraid tx wasm instantiate $code_id \"{}\" --label 'testing' --from validator1 --home $NODE_HOME -b block --no-admin $ARGS"
...
$validator1_command "oraid tx gov submit-proposal set-gasless $contract_address --title 'gasless' --description 'gasless' --deposit 10000000orai $ARGS"
...
$validator1_command "oraid tx gov vote $proposal_id yes $ARGS"
