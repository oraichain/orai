...
func (s *upgradeTestSuite) TestCurrentBin() {
	...
	cfg := oraivisor.Config{Home: home, Name: "dummyd"}
...
func (s *upgradeTestSuite) assertCurrentLink(cfg oraivisor.Config, target string) {
...
func (s *upgradeTestSuite) TestDoUpgradeNoDownloadUrl() {
	home := copyTestData(s.T(), "validate")
	cfg := &oraivisor.Config{Home: home, Name: "dummyd", AllowDownloadBinaries: true}
	...
	for _, name := range []string{"missing", "nobin", "noexec"} {
		info := &oraivisor.UpgradeInfo{Name: name}
		err = oraivisor.DoUpgrade(cfg, info)
	...
	for _, upgrade := range []string{"chain2", "chain3"} {
		// now set it to a valid upgrade and make sure CurrentBin is now set properly
		info := &oraivisor.UpgradeInfo{Name: upgrade}
		err = oraivisor.DoUpgrade(cfg, info)
		s.Require().NoError(err)
		// we should see current point to the new upgrade dir
...
func (s *upgradeTestSuite) TestOsArch() {
	...
	s.Require().Equal("linux/amd64", oraivisor.OSArch())
}
...
func (s *upgradeTestSuite) TestGetDownloadURL() {
	...
	cases := map[string]struct {
		...
		"follow reference": {
			info: ref,
			url:  "https://github.com/cosmos/cosmos-sdk/raw/aa5d6140ad4011bb33d472dca8246a0dcbe223ee/oraivisor/testdata/repo/zip_directory/autod.zip?checksum=sha256:3784e4574cad69b67e34d4ea4425eff140063a3870270a301d6bb24a098a27ae",
		},
	...
	for _, tc := range cases {
		url, err := oraivisor.GetDownloadURL(&oraivisor.UpgradeInfo{Info: tc.info})
		if tc.isErr {
			s.Require().Error(err)
		} else {
			s.Require().NoError(err)
...
func (s *upgradeTestSuite) TestDownloadBinary() {
	...
	for _, tc := range cases {
		...
		upgrade := "amazonas"
		info := &oraivisor.UpgradeInfo{
			Name: upgrade,
			Info: fmt.Sprintf(`{"binaries":{"%s": "%s"}}`, oraivisor.OSArch(), url),
		}

		err = oraivisor.DownloadBinary(cfg, info)
		if !tc.canDownload {
			s.Require().Error(err)
			return
		}
		...
		err = oraivisor.EnsureBinary(cfg.UpgradeBin(upgrade))
		if tc.validBinary {
			s.Require().NoError(err)
		} else {
			s.Require().Error(err)
