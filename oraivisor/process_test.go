...
func (s *processTestSuite) TestLaunchProcess() {
	...
	doUpgrade, err := oraivisor.LaunchProcess(cfg, args, &stdout, &stderr)
	s.Require().NoError(err)
...
func (s *processTestSuite) TestLaunchProcessWithDownloads() {
	...
	cfg := &oraivisor.Config{Home: home, Name: "autod", AllowDownloadBinaries: true}
	...
	s.Require().True(doUpgrade)
	s.Require().Equal("", stderr.String())
	s.Require().Equal("Preparing auto-download some args\n"+`ERROR: UPGRADE "chain2" NEEDED at height: 49: {"binaries":{"linux/amd64":"https://github.com/cosmos/cosmos-sdk/raw/51249cb93130810033408934454841c98423ed4b/oraivisor/testdata/repo/zip_binary/autod.zip?checksum=sha256:dc48829b4126ae95bc0db316c66d4e9da5f3db95e212665b6080638cca77e998"}} module=main`+"\n", stdout.String())
	...
	s.Require().True(doUpgrade)
	s.Require().Equal("", stderr.String())
	s.Require().Equal("Chain 2 from zipped binary link to referral\nArgs: run --fast\n"+`ERROR: UPGRADE "chain3" NEEDED at height: 936: https://github.com/cosmos/cosmos-sdk/raw/0eae1a50612b8bf803336d35055896fbddaa1ddd/oraivisor/testdata/repo/ref_zipped?checksum=sha256:0a428575de718ed3cf0771c9687eefaf6f19359977eca4d94a0abd0e11ef8e64 module=main`+"\n", stdout.String())
