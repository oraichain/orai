            ...
            orai.getAccounts(accAddress).then(acc => {
              ...
              api.addIPFS(ipfsForm, (isSuccess, response, error) => {
                if (isSuccess) {
                  ...
                  api.createFormUnsignedTx(form, api.paths.CLASSIFICATION_REQ, (isSuccess, response, error) => {
                    if (isSuccess) {
                          ...
                          api.getRequestId(txHash, (isSuccess, response, error) => {
                            if (isSuccess) {
                              ...
                              for (let i = 0; i < msgs.length; i++) {
                                ...
                                if (msgs[i].type === constants.MESSAGE_TYPE.SET_CLASSIFICATION_REQUEST) {
                                  let resp = {
                                    requestId: [],
                                    validatorAddrs: [],
                                    blockHeight: "",
