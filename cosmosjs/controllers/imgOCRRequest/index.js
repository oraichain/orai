...
const controller = require("./ocrController.js");
...
router.post("/img_ocr", upload.single('image'), [
...
router.post("/img_ocr_hash", [
  check("oscript_name").isLength({ min: 3, max: 99 }),
  check("input").isBase64(),
  check("expected_output").isBase64(),
  check("fees").isNumeric(),
  check("validator_count").isNumeric().isInt({ min: 1 }),
  check("image_hash").isLength({ min: 3, max: 99 }),
  check("image_name").isLength({ min: 3, max: 99 })
], controller.getOCRHash);
