...
func (k Keeper) AllocateTokens(ctx sdk.Context, prevVotes []abci.VoteInfo) {
	...
	for _, testCase := range rewardObj.TestCases {
		...
		k.supplyKeeper.SendCoinsFromModuleToAccount(ctx, k.feeCollectorName, testCase.Owner, testCase.Fees)
		remaining = remaining.Sub(sdk.NewDecCoinsFromCoins(testCase.Fees...))
	}

	// reward for test cases that contribute
	for _, dataSource := range rewardObj.DataSources {
		// send coins to data source owner addresses
		k.supplyKeeper.SendCoinsFromModuleToAccount(ctx, k.feeCollectorName, dataSource.Owner, dataSource.Fees)
		remaining = remaining.Sub(sdk.NewDecCoinsFromCoins(dataSource.Fees...))
	}
