...
// handleMsgSetClassificationRequest is a function message setting a new AI request
func handleMsgSetClassificationRequest(ctx sdk.Context, k keeper.Keeper, msg types.MsgSetClassificationRequest) (*sdk.Result, error) {
	...
	// TODO: Define your msg events
	// Emit an event describing a data request and asked validators.
	event := sdk.NewEvent(types.EventTypeSetClassificationRequest)
	event = event.AppendAttributes(
		sdk.NewAttribute(types.AttributeRequestID, string(request.RequestID[:])),
	)
	for _, validator := range validators {
