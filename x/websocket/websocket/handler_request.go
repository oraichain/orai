...
func handleAIRequestLog(c *Context, log sdk.ABCIMessageLog) {
	fmt.Info(":delivery_truck: Processing incoming request event before checking validators")
	...
	for _, validator := range validators {
		fmt.Info(":delivery_truck: validator: %s", validator)
	...
	if !hasMe {
		fmt.Debug(":next_track_button: Skip request not related to this validator")
		return
	}

	fmt.Info(":delivery_truck: Processing incoming request event")

	req, err := GetEventAIRequest(log)
	if err != nil {
		fmt.Error(":skull: Failed to parse raw requests with error: %s", err.Error())
	}
	...
	if err != nil {
		fmt.Error(":skull: Failed to parse ai data sources and test cases with error: %s", err.Error())
	}
	...
	for i := range testCases {
		...
		for j := range aiDataSources {
			...
			if err != nil {
				fmt.Error(":skull: failed to execute test case 1st loop: %s", err.Error())
			}
	...
	for i := range dataSourceResultsTest {
		...
		if dataSourceResultsTest[i].GetStatus() == types.ResultSuccess {
			...
			if err != nil {
				fmt.Error(":skull: failed to execute data source script: %s", err.Error())
			}
			// collect test case result from the script
			result := trimResultEscapeChars(outDataSource)
			//result = strings.TrimSuffix(result, "\r")
			fmt.Info("star: result from data sources: ", result)
			// By default, we consider returning null as failure. If any datasource does not follow this rule then it should not be used by any oracle scripts.
	...
	fmt.Info("star: final result after trimming: ", finalResultStr)
	...
	if len(finalResultStr) == 0 {
		...
		if err != nil {
			fmt.Error(":skull: failed to aggregate results: %s", err.Error())
		}
