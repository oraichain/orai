...
type websocketKeeper interface {
	GetReports(ctx sdk.Context, rid string) (reports []websocket.Report)
	HasReport(ctx sdk.Context, id string, val sdk.ValAddress) bool
	GetReportsBlockHeight(ctx sdk.Context, blockHeight int64) (reports []websocket.Report)
	ValidateReport(ctx sdk.Context, rep websocket.Report, req airequest.AIRequest) error
	GetKeyResultSuccess() string
	NewValidator(address sdk.ValAddress, votingPower int64, status string) websocket.Validator
	NewValResult(val websocket.Validator, result []byte, status string) websocket.ValResult
}

// AIRequestKeeper defines the expected airequest keeper
type AIRequestKeeper interface {
	GetAIRequest(ctx sdk.Context, id string) (*airequest.AIRequest, error)
}
