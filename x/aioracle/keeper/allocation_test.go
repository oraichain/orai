...
// 	k := keeper.NewKeeper(app.AppCodec(), app.GetKey("staking"), nil, app.GetSubspace(stakingtypes.ModuleName), app.StakingKeeper, app.BankKeeper, app.DistrKeeper, app.AccountKeeper, nil, aioracleKeeper, types.FeeCollectorName)

// 	// wrap keeper in a test keeper for test functions
// 	testKeeper := keeper.NewTestKeeper(*k, app.AppCodec(), app.GetKey("staking"), nil, app.GetSubspace(stakingtypes.ModuleName), app.StakingKeeper, app.BankKeeper, app.DistrKeeper, app.AccountKeeper, nil, aioracleKeeper, types.FeeCollectorName)

// 	id := ksuid.New().String()
// 	testKeeper.aioracleKeeper.Setaioracle(ctx, id, &aioracle.aioracle{RequestID: id})

// 	aioracle, err := testKeeper.aioracleKeeper.Getaioracle(ctx, id)

// 	require.NoError(t, err)
// 	require.Equal(t, &aioracle.aioracle{RequestID: id}, aioracle)
