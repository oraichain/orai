...
import (
	...
	artypes "github.com/oraichain/orai/x/aioracle/types"
...
type Subscriber struct {
	...
	config *AIOracleConfig
	log    *Logger
}

// NewQuerier return querier implementation
func NewSubscriber(cliCtx *client.Context, config *AIOracleConfig) *Subscriber {
	...
	for _, log := range logs {
		for _, ev := range log.Events {
			...
			case artypes.EventTypeRequestWithData:
				_, err := subscriber.handleAIOracleLog(queryClient, &ev)
				if err != nil {
					return err
				}
			default:
...
func (subscriber *Subscriber) Subscribe() error {
	...
	subscriber.log.Info(":beer: aioracle Subscribing with validator: %s, exit on error: %b ...",
