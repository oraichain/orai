...
func keysCmd(c *Context) *cobra.Command {
	...
	cmd.AddCommand(keysDeleteAllCmd(c))
...
func keysDeleteAllCmd(c *Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "delete-all",
		Aliases: []string{"d"},
		Short:   "Delete all keys from the keychain",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, key := range keys {
				if err := keybase.Delete(key.GetName(), "", true); err != nil {
					return err
				}
				fmt.Printf("Deleted key: %s\n", key.GetName())
			}
