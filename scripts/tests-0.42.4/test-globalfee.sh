...
add_globalfee() {
   ...
   proposal_id=$(oraid query tx $proposal_txhash --output json | jq '.logs[0].events[3].attributes[1].value | tonumber')
...
globalfee_error_message=$(oraid tx send validator1 orai1kzkf6gttxqar9yrkxfe34ye4vg5v4m588ew7c9 1orai $VALIDATOR1_ARGS --keyring-backend test --chain-id testing --gas 200000 -y --output json | jq '.raw_log')
