...
proto_dirs=$(find $MODULE_SDK_DIR $COSMOS_SDK_DIR/proto -path -prune -o -name '*.proto' -print0 | xargs -0 -n1 dirname | sort | uniq)
...
fs.readdirSync(baseModuleDir).forEach(dir=>{
   obj.apis.push({
        url: baseModuleDir + '/' + dir + '/types/query.swagger.json',
        operationIds: {
            rename: {
